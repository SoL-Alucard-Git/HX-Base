namespace Aisino.Fwkp.Fptk.Form
{
    ...
    using Common.Forms;
    public class InvoiceForm_DZ : InvoiceForm
    {
        ...
        internal InvoiceForm_DZ(FPLX fplx, string fpdm, string fphm)
        {
            this.tip = new ToolTip();
            this.blueJe = string.Empty;
            this.log = LogUtil.GetLogger<InvoiceForm_DZ>();
            this.djfile = "";
        ...
        internal InvoiceForm_DZ(FPLX fplx, ZYFP_LX zyfplx, string fpdm, string fphm)
        {
            this.tip = new ToolTip();
            this.blueJe = string.Empty;
            this.log = LogUtil.GetLogger<InvoiceForm_DZ>();
            ...
            {
                if (((int)zyfplx == 0) && !base.TaxCardInstance.QYLX.ISPTFPDZ)
                {
                    string[] textArray3 = new string[] { " 无电子增值税普通发票授权。" };
            ...
            else if (((int)fplx == 0x33) && ((int)zyfplx == 9))
            {
                this.isNcpsgfp = true;
                this.zyfplx = zyfplx;
            }
        ...
        public InvoiceForm_DZ(FPLX fplx, string fpdm, string fphm, int flag, byte[] value)
        {
            this.tip = new ToolTip();
            this.blueJe = string.Empty;
            this.log = LogUtil.GetLogger<InvoiceForm_DZ>();
            ...
            if (flag == 1)
            {
                string str = "Aisino.Fwkp.Invoice.ActiveX.Invoice_DZ" + fpdm + fphm;
        ...
        protected override void _comboBox_SLV_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if ((((selectedItem.fplx == 0) && ((int)selectedItem.Zyfplx == 1)) || (((int)base._fpxx.Zyfplx == 1) && ((int)selectedItem.Zyfplx == 0))) && !base._fpxx.SetZyfpLx(selectedItem.Zyfplx))
                {
                ...
                else
                {
                    ...
                    else
                    {
                        if (base._fpxx.Fplx == 0)
                        {
                            ...
                            if (((selectedItem != null) && (selectedItem.fplx == 0)) && ((int)selectedItem.Zyfplx == 1))
                            {
        ...
        protected override void _dataGridView_RowsRemoved(object sender, DataGridViewRowsRemovedEventArgs e)
        {
            ...
            bool flag = false;
            if (((base._fpxx.IsRed && (base._fpxx.Fplx == 0)) && ((this.blueJe.Trim() != "") && (this.prevCmbSlv != null))) && (((int)((SLV)this.prevCmbSlv).Zyfplx) == 1))
            {
                flag = true;
            }
        ...
        protected override Dictionary<FPLX, List<SLV>> _GetSLvList()
        {
            ...
            if (base._fpxx != null)
            {
                ...
                List<SLV> list2 = dictionary[(FPLX)0x33];
                list.Clear();
                list2.Clear();
                ...
                list3.Sort(new Comparison<string>(InvoiceForm_DZ.CompareSlvDesc));
                if (base._fpxx.Fplx == 0)
                {
                ...
                if ((int)base._fpxx.Fplx != 0x33)
                {
                    return dictionary;
                }
                if (this.isNcpsgfp)
                {
                    list2.Add(new SLV((FPLX)0x33, (ZYFP_LX)9, "0.00", "0%", "0%"));
                    return dictionary;
                }
                foreach (string str4 in list3)
                {
                    ...
                    if (double.TryParse(str4, out num3))
                    {
                        if (num3 == 0.0)
                        {
                            list2.Add(new SLV((FPLX)0x33, (ZYFP_LX)0, "0.00", "0%", "0%"));
                        }
                        else
                        {
                            string str5 = ((num3 * 100.0)).ToString() + "%";
                            list2.Add(new SLV((FPLX)0x33, (ZYFP_LX)0, str4, str5, str5));
                        }
                    }
                }
            }
        ...
        internal bool FillDjxx(Djfp djfp, bool isRed = false)
        {
            ...
            if (!this.fpm.ChargeAllInfo(fpxx, this.isWM(), true))
            {
                djfp.Fpxx = null;
                string errorTip = this.fpm.GetErrorTip();
                object[] objArray1 = new object[] { base.TaxCardInstance.TaxClock.ToString("yyyy-MM-dd HH:mm:ss"), djfp.Djh, 0, errorTip };
                AutoImport.KpResult = string.Format("[{0}] 单据号:{1},开具结果:{2},开具失败原因:{3}", objArray1);
                AutoImport.returncode = "0001";
                AutoImport.returnmsg = errorTip;
                return false;
            }
            ...
            if (flag)
            {
                if (isRed)
                {
                    base._fpxx.Bz=NotesUtil.GetRedInvNotes(fpxx.blueFpdm, fpxx.blueFphm) + " " + bz;
        ...
        private void GetNextFp()
        {
            if (this.fpm.CanInvoice(base._fpxx.Fplx))
            {
                ...
                if (current != null)
                {
                    ...
                    string str7 = PropertyUtil.GetValue("INV-HSJBZ", "0");
                    byte[] destinationArray = new byte[0x20];
                    byte[] sourceArray = Invoice.TypeByte;
                    Array.Copy(sourceArray, 0, destinationArray, 0, 0x20);
                    byte[] buffer2 = new byte[0x10];
                    ...
                    byte[] buffer3 = AES_Crypt.Encrypt(Encoding.Unicode.GetBytes("KP" + DateTime.Now.ToString("F")), destinationArray, buffer2);
                    Invoice.IsGfSqdFp_Static=false;
                    base._fpxx = new Invoice(false, false, str7.Equals("1"), base._fpxx.Fplx, buffer3, null);
        ...
        {
            base.XmlComponentFile=@"..\Config\Components\Aisino.Fwkp.Fpkj.Form.DZFPtiankai_new\Aisino.Fwkp.Fpkj.Form.DZFPtiankai_new.xml";
            ...
            this.tool_sddc = base._xmlComponentLoader.GetControlByName<ToolStripButton>("tool_sddc");
        ...
        private void InvoiceForm_Init(FPLX fplx, string fpdm, string fphm)
        {
            ...
            if (((int)fplx == 0x33) && ((this.zyfplx == (ZYFP_LX)9) || (this.zyfplx == (ZYFP_LX)8)))
            {
                base._fpxx.SetZyfpLx(this.zyfplx);
            }
                                    ...
                                    else
                                    {
                                        ...
                                        if (fp.isRed)
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(str5))
                                            {
                                                ...
                                                else if (base._fpxx.Fplx == (FPLX)0x33)
                                                {
                                                    if (str5.Length == 20)
                                                    {
                                            ...
                                            if ((fp.isRed && (base._fpxx.Fplx == (FPLX)0x33)) && ((fp.blueFpdm.Length == 0) || (fp.blueFphm.Length == 0)))
                                            {
        ...
        public void RefreshData(bool isRed)
        {
            ...
            string str5 = PropertyUtil.GetValue("INV-HSJBZ", "0");
            byte[] destinationArray = new byte[0x20];
            byte[] sourceArray = Invoice.TypeByte;
            Array.Copy(sourceArray, 0, destinationArray, 0, 0x20);
            byte[] buffer2 = new byte[0x10];
            ...
            byte[] buffer3 = AES_Crypt.Encrypt(Encoding.Unicode.GetBytes("KP" + DateTime.Now.ToString("F")), destinationArray, buffer2);
            Invoice.IsGfSqdFp_Static=false;
            base._fpxx = new Invoice(isRed, false, str5.Equals("1"), base._fpxx.Fplx, buffer3, null);
            base._fpxx.Bmbbbh=base.getbmbbbh();
            base._fpxx.SetZyfpLx(this.zyfplx);
            base._fpxx.AddSpxx(this._SetDefaultSpsm(), this._SetDefaultSpsLv(), this.zyfplx);
            base._DataGridView.Rows.Clear();
            ...
            base._HsjbzButton.Checked = str5.Equals("1");
            if (this.isNcpsgfp)
            {
                this.com_xfmc.Edit=(EditStyle)1;
                this.com_xfsbh.Edit= (EditStyle)1;
                this.com_xfdzdh.Edit= (EditStyle)1;
                this.com_xfzh.Edit= (EditStyle)1;
            }
            else
            {
                this.com_gfmc.Edit= (EditStyle)1;
                this.com_gfsbh.Edit= (EditStyle)1;
                this.com_gfdzdh.Edit= (EditStyle)1;
                this.com_gfzh.Edit= (EditStyle)1;
            }
        }
        ...
        private void SetDrfpQdbz(Fpxx fpxx)
        {
            ...
            {
                if ((base._fpxx.Fplx == (FPLX)0x33) && (fpxx.Mxxx.Count > 7))
                {
                    base._fpxx.SetQdbz(true);
                }
            }
                    ...
                    case "不征税":
                    {
                            IEnumerator enumerator = base.comboBox_SLV.Items.GetEnumerator();
                        {
                            while (enumerator.MoveNext())
                            {
                                ...
                                if (base.comboBox_SLV.Items[num3].ToString() == str4)
                                {
                                    flag = true;
                                    break;
                                }
        ...
        Label_0352:
            ...
            if (((base._fpxx.Fplx == (FPLX)0x33) && (num2.ToString() == "0.05")) && (str == "True"))
            {
            ...
            base._fpxx.Hsjbz=spxx[8].ToString().Equals("True");
        ...
        private void SetTitleFont()
        {
            ...
            this.lab_title.Font = new Font("华文楷体", 24f, FontStyle.Bold);
            this.lblDq.Font = new Font("华文楷体", 10f);
        }
        ...
        private void ShowCopy(Fpxx fp)
        {
            ...
            else
            {
                ...
                if (((base._fpxx.Fplx == 0) && (this.zyfplx == 0)) && ((((int)fp.Zyfplx == 2) || ((int)fp.Zyfplx == 3)) || (((int)fp.Zyfplx == 4) || ((int)fp.Zyfplx == 5))))
                {
                    base._fpxx.SetZyfpLx(this.zyfplx);
                }
        ...
        private void SLVLIST_CHANGE(int rowidx)
        {
            ...
            if (base._fpxx != null)
            {
                List<SLV> list2 = dictionary2[(FPLX)0x33];
                list2.Clear();
                list.Sort();
                list.Reverse();
                foreach (double num2 in list)
                {
                    if (num2 == 0.0)
                    {
                        if (!FLBM_lock.isFlbm())
                        {
                            list2.Add(new SLV((FPLX)0x33, (ZYFP_LX)0, "0.00", "免税", "免税"));
                        }
                        else
                        {
                            ...
                            if (str.Contains("不征税"))
                            {
                                list2.Add(new SLV((FPLX)0x33, (ZYFP_LX)0, "0.00", "不征税", "不征税"));
                            }
                        }
                    }
                    else if (num2 != 0.015)
                    {
                        ...
                        list2.Add(new SLV((FPLX)0x33, (ZYFP_LX)0, num2.ToString(), str2, str2));
                    }
                }
            }
        ...
        {
            new ImportSet((FPLX)0x33).ShowDialog();
        }
                        ...
                        if (import.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            if ((djfp != null) && (djfp.Fpxx != null))
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        MessageManager.ShowMsgBox(this.fpm.GetErrorTip());
                                    }
                                    else
                                    {
                                        MessageManager.ShowMsgBox(this.fpm.Code(), this.fpm.CodeParams());
        ...
        private void tool_print_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!base._onlyShow)
                {
                    ...
                    if (base._fpxx.IsRed)
                    {
                        if ((base._fpxx.Fplx == (FPLX)0x33) && !this.CheckPTHZFP())
                        {
                            return;
                        }
                ...
                else
                {
                    ...
                    {
                        string[] textArray3 = new string[] { "购买方纳税人识别号" };
        ...
        {
            public static readonly InvoiceForm_DZ.serializeClass instance = new InvoiceForm_DZ.serializeClass();
