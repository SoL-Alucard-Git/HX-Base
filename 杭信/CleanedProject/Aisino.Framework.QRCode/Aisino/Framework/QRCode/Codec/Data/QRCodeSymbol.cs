...
{
    using Aisino.Framework.QRCode.Codec.Ecc;
    using Aisino.Framework.QRCode.Codec.Reader.Pattern;
    ...
    public class QRCodeSymbol
    {
        ...
        public virtual bool isInFunctionPattern(int targetX, int targetY)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    if (!((((j != 0) || (i != 0)) && ((j != (length - 1)) || (i != 0))) ? ((j == 0) && (i == (length - 1))) : true) && ((Math.Abs((int) (alignmentPattern[j][i].X - targetX)) < 3) && (Math.Abs((int) (alignmentPattern[j][i].Y - targetY)) < 3)))
                    {
                        return true;
                    }
                }
