...
using Aisino.Framework.QRCode.Properties;

namespace Aisino.Framework.QRCode.Codec
{
    public class QRCodeEncoder
    {
        ...
        private static sbyte[] calculateRSECC(sbyte[] codewords, sbyte rsEccCodewords, sbyte[] rsBlockOrder, int maxDataCodewords, int maxCodewords)
        {
            ...
            for (index = 0; (index < maxDataCodewords) && (num4 < rsBlockOrder.Length); index++)
            {
        ...
        0x22d8, 0x2498, 0x2678, 0x2830, 0x2a50, 0x2c90, 0x2ef0, 0x3170, 0x3410
            ...
            {
                maxCodewords = numArray4[num7][this.qrcodeVersion];
            }
            else
            {
                ...
                while (num4 <= 40)
                {
                    ...
                    {
                        maxCodewords = numArray4[num7][num4];
                        break;
                    }
                    this.qrcodeVersion++;
                    num4++;
        ...
        3, 3, 3, 0, 0, 0, 0, 0, 0
            ...
            int maxDataCodewords = maxCodewords >> 3;
            ...
            if (num6 <= (maxCodewords - 4))
            {
                data[index] = 0;
                bits[index] = 4;
            }
            else if (num6 < maxCodewords)
            {
                data[index] = 0;
                bits[index] = (sbyte)(maxCodewords - num6);
            }
            else if (num6 > maxCodewords)
            {
            ...
            {
                int num22 = (k + (num9 * 8)) - 1;
                matrixContent[target[num22] & 0xff][numArray8[num22] & 0xff] = (sbyte)(0xff ^ numArray9[num22]);
            }
            sbyte num23 = selectMask(matrixContent, numArray6[this.qrcodeVersion] + (num9 * 8));
        ...
        private static sbyte[] divideDataBy8Bits(int[] data, sbyte[] bits, int maxDataCodewords)
        {
            ...
            int num2 = ((num5 - 1) / 8) + 1;
            sbyte[] numArray = new sbyte[Math.Max(num2, maxDataCodewords)];
        ...
        public virtual Bitmap Encode(string content, Encoding encoding)
        {
            ...
            for (int i = 0; i < flagArray.Length; i++)
            {
                for (int j = 0; j < flagArray.Length; j++)
                {
                    if (flagArray[j][i])
                    {
                        graphics.FillRectangle(brush, (int)(j * this.qrCodeScale), (int)(i * this.qrCodeScale), (int)(this.qrCodeScale - 1), (int)(this.qrCodeScale - 1));
                    }
                }
            }
            return image;
        ...
        // Properties
        public virtual System.Drawing.Color QRCodeBackgroundColor { get; set; }
        public virtual ENCODE_MODE QRCodeEncodeMode { get; set; }
        public virtual ERROR_CORRECTION QRCodeErrorCorrect { get; set; }
        public virtual System.Drawing.Color QRCodeForegroundColor { get; set; }
