namespace Aisino.Fwkp.Bmgl.Forms
{
    ...
    public class BMFYXMSelect : BaseForm
    {
        ...
        {
            this.SelectedBM = "";
            this.feiyongManager = new BMFYXMManager();
            this.log = LogUtil.GetLogger<BMFYXMSelect>();
            this.SelectedSJBM = "";
        ...
        public BMFYXMSelect(string keyWord)
        {
            this.SelectedBM = "";
            this.feiyongManager = new BMFYXMManager();
            this.log = LogUtil.GetLogger<BMFYXMSelect>();
        ...
        {
            this.feiyongManager.CurrentPage = e.PageNO;
            this.feiyongManager.Pagesize = e.PageSize;
            this.dataSet = this.feiyongManager.QueryData(e.PageSize, e.PageNO);
            this.aisinoDataGrid1.DataSource = this.dataSet;
            this.ChangeCanSelect();
        }

        private void BMFYXMSelect_Load(object sender, EventArgs e)
        {
            ...
            this.treeViewBM1.RootNodeString = "费用项目编码";
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                this.feiyongManager.CurrentPage = 1;
                this.KeyWord = this.textBoxWaitKey.Text.Trim();
                this.dataSet = this.feiyongManager.QueryByKey(this.KeyWord, this.feiyongManager.Pagesize, this.feiyongManager.CurrentPage);
                this.aisinoDataGrid1.DataSource = this.dataSet;
            }
            catch (Exception exception)
            {
        ...
        private void ChangeCanSelect()
        {
            if (this.aisinoDataGrid1.Rows.Count > 0)
            {
                if (this.HiKeyWord.Trim().Length > 0)
                {
                    ...
                    {
                        if ((Convert.ToString(row.Cells["MC"].Value).Contains(this.HiKeyWord) || Convert.ToString(row.Cells["BM"].Value).Contains(this.HiKeyWord)) || Convert.ToString(row.Cells["JM"].Value).ToUpper().Contains(str))
                        {
        ...
        private void Initialize()
        {
            ...
            this.textBoxWaitKey.ToolTipText = "输入关键字(编码,名称或简码)";
        ...
        {
            ComponentResourceManager manager = new ComponentResourceManager(typeof(BMFYXMSelect));
            ...
            this.xmlComponentLoader1.Text = "BMFYXMSelect";
            this.xmlComponentLoader1.XMLPath = @"..\Config\Components\Aisino.Fwkp.Bmgl.Forms.BMFYXMSelect\Aisino.Fwkp.Bmgl.Forms.BMFYXMSelect.xml";
            base.AutoScaleDimensions = new SizeF(6f, 12f);
            base.AutoScaleMode = AutoScaleMode.Font;
            base.ClientSize = new Size(0x3e8, 600);
            base.Controls.Add(this.xmlComponentLoader1);
            base.KeyPreview = true;
            base.Name = "BMFYXMSelect";
            base.Tag = manager.GetObject("$this.Tag");
            this.Text = "费用项目选择";
            base.Load += new EventHandler(this.BMFYXMSelect_Load);
        ...
        {
            return this.feiyongManager.listNodes(ParentBM);
        }
        ...
        private bool treeViewBM1_onTreeNodeClick(object sender, TreeViewEventArgs e)
        {
            ...
            this.feiyongManager.CurrentPage = 1;
            this.dataSet = this.feiyongManager.SelectNodeDisplay(this.selectbm, this.feiyongManager.Pagesize, this.feiyongManager.CurrentPage);
            this.aisinoDataGrid1.DataSource = this.dataSet;
            this.ChangeCanSelect();
            return true;
        }
