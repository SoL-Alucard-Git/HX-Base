namespace Aisino.Fwkp.Bmgl.BLL
{
    ...
    internal sealed class BMSFHRManager : IBMBaseManager
    {
        private int _currentPage = 1;
        private DAL.BMSFHRManager bmsfhrDAL = new DAL.BMSFHRManager();
        ...
        public string AddCustomerKP(BMSFHRModel customer, int Addtype)
        {
            ...
            if (str == "0")
            {
                ...
                {
                    MessageManager.ShowMsgBox("INP-235132");
                    return "收/发货人已存在";
                }
        ...
        public string CheckCustomer(BMSFHRModel customer)
        {
            if (customer == null)
            {
                return "e11";
        ...
        public string ExportData(string pathFile, string separator, DataTable khTable = null)
        {
            ...
            {
                writer.WriteLine("{收/发货人编码}[分隔符]\"" + separator + "\"");
        ...
        public BMBaseModel GetModel(string BM)
        {
            ...
            BMSFHRModel model = new BMSFHRModel();
            if (kH.Rows.Count > 0)
            {
        ...
        public ImportResult ImportData(string codeFile)
        {
            ...
            try
            {
                ...
                while (flag)
                {
                    if (strArray[index].StartsWith("{收/发货人编码}") || strArray[index].StartsWith("{发货人编码}"))
                    {
                        string str2 = strArray[index];
                        if (!str2.Contains("\""))
                        {
                            throw new CustomException("此文件首行收/发货人编码没有指定分隔符，分隔符使用双引号标注");
                        }
                    ...
                    else
                    {
                        ...
                        {
                            throw new CustomException("此文件不符合收/发货人编码文本格式");
                        }
                    }
                }
                ...
                Label_0430:
                    ...
                    else if ((model.SH != null) && (((model.SH.Length < 6) || (model.SH.Length > 20)) || (((model.WJ == 1) && (model.SH.Length != 0)) && ("0000" != this.jyCustomer.CheckTaxCode(model.SH, (FPLX)11)))))
                    {
                        str7 = "失败";
                        str8 = "收发货人税号错误" + model.SH;
                        result.Failed++;
                        none = ResultType.Failed;
                    }
                    else if (this.bmsfhrDAL.ExistCusMC(model.BM, model.MC, model.SJBM))
                    {
                        str7 = "失败";
                        str8 = "同级同族已存在相同名称的收/发货人" + model.SH;
                        result.Failed++;
                        none = ResultType.Failed;
                    }
                    else
