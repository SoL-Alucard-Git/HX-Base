namespace Aisino.Framework.Pdf417.Code
{
    ...
    public class PDF417Class
    {
        protected internal const int ABSOLUTE_MAX_TEXT_SIZE = 0x152c;
        protected internal const int AL = 0x1c;
        protected internal const int ALPHA = 0x10000;
        protected internal const int AS = 0x1b;
        private float aspectRatio;
        private int bitColumns;
        protected internal int bitPtr;
        protected internal const int BYTE_MODE = 0x385;
        protected internal const int BYTE_MODE_6 = 0x39c;
        protected internal const int BYTESHIFT = 0x391;
        internal static int[][] CLUSTERS = new int[][] { new int[] { 
        ...
        internal static int[][] ERROR_LEVEL = new int[][] { new int[] { 0x1b, 0x395 }, new int[] { 0x20a, 0x238, 0x2d3, 0x329 }, new int[] { 0xed, 0x134, 0x1b4, 0x11c, 0x286, 0x28d, 0x1ac, 0x17b }, new int[] { 0x112, 0x232, 0xe8, 0x2f3, 0x257, 0x20c, 0x321, 0x84, 0x127, 0x74, 0x1ba, 0x1ac, 0x127, 0x2a, 0xb0, 0x41 }, new int[] { 
        ...
        protected internal const int Ibyte = 0x100000;
        private int lenCodewords;
        protected internal const int LL = 0x1b;
        protected internal const int LOWER = 0x20000;
        protected internal const int MAX_DATA_CODEWORDS = 0x39e;
        protected internal const int MIXED = 0x40000;
        internal static string MIXED_SET = "0123456789&\r\t,:#-.$/+%*=^";
        protected internal const int ML = 0x1c;
        protected internal const int MOD = 0x3a1;
        protected internal const int NUMERIC_MODE = 0x386;
        private int options;
        private byte[] outBits;
        protected internal const int PAL = 0x1d;
        ...
        public const int PDF417_INVERT_BITMAP = 0x80;
        public const int PDF417_USE_ASPECT_RATIO = 0;
        public const int PDF417_USE_ERROR_LEVEL = 0x10;
        public const int PDF417_USE_RAW_CODEWORDS = 0x40;
        protected internal const int PL = 0x19;
        protected internal const int PS = 0x1d;
        ...
        protected internal const int SPACE = 0x1a;
        protected internal const int START_CODE_SIZE = 0x11;
        protected internal const int START_PATTERN = 0x1fea8;
        protected internal const int STOP_PATTERN = 0x3fa29;
        protected internal const int STOP_SIZE = 0x12;
        private byte[] text;
        ...
        protected internal virtual void assemble()
        {
            if (this.segmentList.size() != 0)
            {
                ...
                {
                    Segment segment = this.segmentList[i];
                    char type = segment.type;
                    switch (type)
                    {
                        case 'B':
                            this.codewords[this.cwPtr++] = ((this.getSegmentLength(segment) % 6) != 0) ? 0x385 : 0x39c;
                            this.byteCompaction(segment.start, this.getSegmentLength(segment));
                            break;

                        case 'N':
                            this.codewords[this.cwPtr++] = 0x386;
        ...
        {
            int cwPtr = this.cwPtr;
            int num2 = length / 3;
            this.cwPtr += num2 + 1;
            int num4 = 0;
            while (num4 <= num2)
            {
                this.codewords[cwPtr + num4] = 0;
                num4++;
            }
            this.codewords[cwPtr + num2] = 1;
            length += start;
            for (int i = start; i < length; i++)
            {
                ...
                while (num4 >= 0)
                {
                    this.codewords[cwPtr + num4] *= 10;
                    num4--;
                }
                this.codewords[cwPtr + num2] += this.text[i] - 0x30;
                for (num4 = num2; num4 > 0; num4--)
                {
                    this.codewords[(cwPtr + num4) - 1] += this.codewords[cwPtr + num4] / 900;
                    this.codewords[cwPtr + num4] = this.codewords[cwPtr + num4] % 900;
                }
            }
        }
        ...
        internal virtual void breakString()
        {
            ...
            for (start = 0; start < length; start++)
            {
                ...
                else
                {
                    if (num4 >= 13)
                    {
                        if (index != end)
                        {
                            ch = (char) (this.text[index] & 0xff);
                            flag = (((ch >= ' ') && (ch < '\x007f')) || ((ch == '\r') || (ch == '\n'))) || (ch == '\t');
                            num7 = index;
                            while (num7 < end)
                            {
                                ch = (char) (this.text[num7] & 0xff);
                                if ((flag2 = (((ch >= ' ') && (ch < '\x007f')) || ((ch == '\r') || (ch == '\n'))) || (ch == '\t')) != flag)
                                {
                                    this.segmentList.add(flag ? 'T' : 'B', index, num7);
                                    index = num7;
                                    flag = flag2;
                                }
                                num7++;
                            }
                            this.segmentList.add(flag ? 'T' : 'B', index, end);
                        }
                        this.segmentList.add('N', end, start);
                        index = start;
                    }
                    num4 = 0;
                }
            ...
            if (num4 >= 13)
            {
                this.segmentList.add('N', end, length);
            }
            for (start = 0; start < this.segmentList.size(); start++)
            {
                segment = this.segmentList[start];
                segment2 = this.segmentList[start - 1];
                segment3 = this.segmentList[start + 1];
            ...
            for (start = 0; start < this.segmentList.size(); start++)
            {
                ...
                {
                    flag3 = false;
                    if (!((!this.checkSegmentType(segment2, 'B') || (this.getSegmentLength(segment2) != 1)) ? !this.checkSegmentType(segment2, 'T') : false))
                    {
                        flag3 = true;
                        segment.start = segment2.start;
                        this.segmentList.remove(start - 1);
                        start--;
                    }
                    if (!((!this.checkSegmentType(segment3, 'B') || (this.getSegmentLength(segment3) != 1)) ? !this.checkSegmentType(segment3, 'T') : false))
                    {
                        flag3 = true;
            ...
            for (start = 0; start < this.segmentList.size(); start++)
            {
                ...
                {
                    flag3 = false;
                    if (!((!this.checkSegmentType(segment2, 'T') || (this.getSegmentLength(segment2) >= 5)) ? !this.checkSegmentType(segment2, 'B') : false))
                    {
                        flag3 = true;
                        segment.start = segment2.start;
                        this.segmentList.remove(start - 1);
                    ...
                    if (!((!this.checkSegmentType(segment3, 'T') || (this.getSegmentLength(segment3) >= 5)) ? !this.checkSegmentType(segment3, 'B') : false))
                    {
                        flag3 = true;
                        segment.end = segment3.end;
                        this.segmentList.remove(start + 1);
            ...
            if (((this.segmentList.size() == 1) && ((segment = this.segmentList[0]).type == 'T')) && (this.getSegmentLength(segment) >= 8))
            {
                start = segment.start;
        ...
        internal virtual void byteCompaction(int start, int length)
        {
            ...
            for (int i = start; i < length; i += 6)
            {
                ...
                if (num3 < 6)
                {
                    ...
                    {
                        this.codewords[this.cwPtr++] = this.text[i + j] & 0xff;
                    }
                }
                else
                {
        ...
        protected internal virtual void byteCompaction6(int start)
        {
            ...
            for (int i = start; i < num; i++)
            {
                ...
                while (num5 >= 0)
                {
                    this.codewords[cwPtr + num5] *= 0x100;
                    num5--;
                }
                this.codewords[cwPtr + num3] += this.text[i] & 0xff;
                for (num5 = num3; num5 > 0; num5--)
                {
                    this.codewords[(cwPtr + num5) - 1] += this.codewords[cwPtr + num5] / 900;
                    this.codewords[cwPtr + num5] = this.codewords[cwPtr + num5] % 900;
                }
            }
        }
        ...
        protected internal virtual void calculateErrorCorrection(int dest)
        {
            ...
            int[] numArray = ERROR_LEVEL[this.errorLevel];
            int num = ((int) 2) << this.errorLevel;
            for (num2 = 0; num2 < num; num2++)
            {
                this.codewords[dest + num2] = 0;
            }
            int num3 = num - 1;
            for (num2 = 0; num2 < this.lenCodewords; num2++)
            {
                int num4 = this.codewords[num2] + this.codewords[dest];
                for (int i = 0; i <= num3; i++)
                {
                    int num6 = (num4 * numArray[num3 - i]) % 0x3a1;
                    int num7 = 0x3a1 - num6;
                    this.codewords[dest + i] = (((i == num3) ? 0 : this.codewords[(dest + i) + 1]) + num7) % 0x3a1;
                }
            }
            for (num2 = 0; num2 < num; num2++)
            {
                this.codewords[dest + num2] = (0x3a1 - this.codewords[dest + num2]) % 0x3a1;
            }
        }
        ...
        protected internal virtual void dumpList()
        {
            if (this.segmentList.size() != 0)
            {
                ...
                {
                    Segment segment = this.segmentList[i];
                    int num2 = this.getSegmentLength(segment);
                    char[] chArray = new char[num2];
                    for (int j = 0; j < num2; j++)
                    {
                        chArray[j] = (char) (this.text[segment.start + j] & 0xff);
                        if (chArray[j] == '\r')
                        {
                            chArray[j] = '\n';
                        }
                    }
                    Console.Out.WriteLine(segment.type + new string(chArray));
                }
            }
        }

        protected internal byte[] Filter(string sValue)
        {
            try
            {
                return Encoding.UTF8.GetBytes(sValue);
            }
            catch (Exception)
            {
                return null;
        ...
        protected internal virtual int getTextTypeAndValue(int maxLength, int idx)
        {
            ...
            char ch = (char) (this.text[idx] & 0xff);
            if ((ch >= 'A') && (ch <= 'Z'))
            {
            ...
            int index = MIXED_SET.IndexOf(ch);
            int num2 = PUNCTUATION_SET.IndexOf(ch);
            if ((index < 0) && (num2 < 0))
            {
                return (0x100000 + ch);
            }
        ...
        {
            int index = this.bitPtr / 8;
            int num2 = this.bitPtr - (index * 8);
            this.outBits[index++] = (byte) (this.outBits[index++] | ((byte) (codeword >> (9 + num2))));
            this.outBits[index++] = (byte) (this.outBits[index++] | ((byte) (codeword >> (1 + num2))));
            codeword = codeword << 8;
            this.outBits[index] = (byte) (this.outBits[index] | ((byte) (codeword >> (1 + num2))));
            this.bitPtr += 0x11;
        }
        ...
        protected internal virtual void outCodeword18(int codeword)
        {
            int index = this.bitPtr / 8;
            int num2 = this.bitPtr - (index * 8);
            this.outBits[index++] = (byte) (this.outBits[index++] | ((byte) (codeword >> (10 + num2))));
            this.outBits[index++] = (byte) (this.outBits[index++] | ((byte) (codeword >> (2 + num2))));
            codeword = codeword << 8;
            this.outBits[index] = (byte) (this.outBits[index] | ((byte) (codeword >> (2 + num2))));
            if (num2 == 7)
            {
                this.outBits[++index] = (byte) (this.outBits[++index] | 0x80);
            }
            this.bitPtr += 0x12;
        }
        ...
        {
            int num = 0;
            this.bitColumns = (0x11 * (this.codeColumns + 3)) + 0x12;
            ...
            for (int i = 0; i < this.codeRows; i++)
            {
                ...
                int index = i % 3;
                int[] numArray = CLUSTERS[index];
                this.outStartPattern();
                int num5 = 0;
                switch (index)
                {
                    case 0:
                        num5 = (30 * (i / 3)) + ((this.codeRows - 1) / 3);
                        break;

                    case 1:
                        num5 = ((30 * (i / 3)) + (this.errorLevel * 3)) + ((this.codeRows - 1) % 3);
                        break;

                    default:
                        num5 = ((30 * (i / 3)) + this.codeColumns) - 1;
                        break;
                }
                this.outCodeword(numArray[num5]);
        ...
        {
            this.outCodeword17(0x1fea8);
        }

        protected internal virtual void outStopPattern()
        {
            this.outCodeword18(0x3fa29);
        }
        ...
        public virtual void paintCode()
        {
            ...
            {
                if (((this.lenCodewords > 0x39e) || (this.lenCodewords < 1)) || (this.lenCodewords != this.codewords[0]))
                {
            ...
            else
            {
                ...
                if (this.text.Length > 0x152c)
                {
                    throw new IndexOutOfRangeException("The text is too big.");
                }
                this.segmentList = new SegmentList(this);
                this.breakString();
                this.assemble();
                this.segmentList = null;
                this.codewords[0] = this.lenCodewords = this.cwPtr;
            }
            int num = maxPossibleErrorLevel(0x3a0 - this.lenCodewords);
            ...
            int num2 = ((int) 2) << this.errorLevel;
            bool flag = (this.options & 4) == 0;
            bool flag2 = false;
            int maxSquare = this.lenCodewords + num2;
            if ((this.options & 1) != 0)
            {
                maxSquare = this.codeColumns * this.codeRows;
                if (maxSquare > 0x3a0)
                {
                    maxSquare = this.MaxSquare;
                }
                if (maxSquare < (this.lenCodewords + num2))
                {
                    maxSquare = this.lenCodewords + num2;
                }
                else
                {
                    flag2 = true;
            ...
            else if ((this.options & 6) == 0)
            {
                ...
                double num6 = (73f * this.aspectRatio) - 4f;
                double num5 = (-num6 + Math.Sqrt((num6 * num6) + (((68f * this.aspectRatio) * (this.lenCodewords + num2)) * this.yHeight))) / ((double) (34f * this.aspectRatio));
            ...
            if (!flag2)
            {
                if (flag)
                {
                    this.codeRows = ((maxSquare - 1) / this.codeColumns) + 1;
                    if (this.codeRows < 3)
                    {
                        this.codeRows = 3;
                    }
                    ...
                    {
                        this.codeRows = 90;
                        this.codeColumns = ((maxSquare - 1) / 90) + 1;
                    }
                }
                else
                {
                    this.codeColumns = ((maxSquare - 1) / this.codeRows) + 1;
                    if (this.codeColumns > 30)
                    {
                        this.codeColumns = 30;
                        this.codeRows = ((maxSquare - 1) / 30) + 1;
                    }
                }
                maxSquare = this.codeRows * this.codeColumns;
            }
            if (maxSquare > 0x3a0)
            {
                maxSquare = this.MaxSquare;
            }
            this.errorLevel = maxPossibleErrorLevel(maxSquare - this.lenCodewords);
            num2 = ((int) 2) << this.errorLevel;
            int num4 = (maxSquare - num2) - this.lenCodewords;
        ...
        public virtual void setText(byte[] text)
        {
            this.text = text;
        }
        ...
        protected internal virtual void textCompaction(int start, int length)
        {
            ...
            for (int i = start; i < length; i++)
            {
                num4 = this.getTextTypeAndValue(length, i);
                if ((num4 & num) != 0)
                {
                ...
                else
                {
                    switch (num)
                    {
                        case 0x10000:
                            ...
                            else if (((this.getTextTypeAndValue(length, i + 1) & this.getTextTypeAndValue(length, i + 2)) & 0x80000) != 0)
                            {
                                numArray[num2++] = 0x1c;
                                numArray[num2++] = 0x19;
                                numArray[num2++] = num4 & 0xff;
                        ...
                        case 0x20000:
                            if ((num4 & 0x10000) != 0)
                            {
                                if (((this.getTextTypeAndValue(length, i + 1) & this.getTextTypeAndValue(length, i + 2)) & 0x10000) != 0)
                                {
                                    numArray[num2++] = 0x1c;
                                    numArray[num2++] = 0x1c;
                                    num = 0x10000;
            ...
            while (num2 < length)
            {
                ...
                if (num4 >= 30)
                {
                    this.codewords[this.cwPtr++] = num4;
                    this.codewords[this.cwPtr++] = numArray[num2++];
                }
                else
                {
                    this.codewords[this.cwPtr++] = (num4 * 30) + numArray[num2++];
                }
            }
        }
        ...
        internal protected class Segment
        {
            private PDF417Class enclosingInstance;
            public int end;
            public int start;
            public char type;

            public Segment(PDF417Class enclosingInstance, char type, int start, int end)
            {
            ...
            private void InitBlock(PDF417Class enclosingInstance)
            {
                this.enclosingInstance = enclosingInstance;
            }

            public PDF417Class Enclosing_Instance
            {
                get
                {
        ...
        internal protected class SegmentList
        {
            private PDF417Class enclosingInstance;
            protected internal ArrayList list;

            public SegmentList(PDF417Class enclosingInstance)
            {
                this.InitBlock(enclosingInstance);
            }
            ...
            {
                this.list.Add(new PDF417Class.Segment(this.enclosingInstance, type, start, end));
            }
            ...
            public virtual PDF417Class.Segment this[int idx]
            {
                get
                {
                    if ((idx < 0) || (idx >= this.list.Count))
                    ...
                    return (PDF417Class.Segment) this.list[idx];
                }
            }
        }
    }
